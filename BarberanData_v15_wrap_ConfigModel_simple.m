...
%     pval_rs = pval_rs - dummy;
    ...
    for i = 1:n
        for j = i:n
            Pij = (sum(network(i,:))*sum(network(j,:)))/((2*m)-1);
            if randmx(i,j)<Pij
                configmx(i,j) = configmx(i,j) + 1;
                configmx(j,i) = configmx(j,i) + 1;
            end;
        end;
    end;
    ...
    new_network = configmx;
    
    n_nodes(z) = sum( (sum( new_network ) > 0));
    %n
    %n_nodes
    n_edges(z) = sum( sum( new_network )) /2;
    %m
    %n_edges
...
%     avgdeg_includingzeros(z) = (2*n_edges(z))./n;

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %DIAMETER
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     dist = graphallshortestpaths(sparse(new_network));
...
%     apl(z) = mean(dist2);
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    network_upper_tri = triu(new_network,1);
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     b = triu(new_network^2,1);
%     c2 = sum(b); % number of connected triples
%     c1 = sum(b.*new_network); % 3x number of triangles
...
% ylabel('frequency over 2000 trials')
% xlim([0 10])
% title('Distribution of average degrees, 2000 reps, rho=0.36, true data')
...
% csvwrite('ChungLuMethod_apl.csv',apl)
% csvwrite('ChungLuMethod_cc.csv',cc)
% csvwrite('ChungLuMethod_avgdeg.csv',avgdeg)
% csvwrite('ChungLuMethod_diams.csv',diams)
% csvwrite('ChungLuMethod_mod.csv',modular)
% csvwrite('ChungLuMethod_avgdeg_withsingletons.csv', avgdeg_includingzeros)
