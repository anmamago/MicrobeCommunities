...
tic
numreps = 1;
ThresholdTics = 41; %this should be one more than the value you want: 10 -> 11
network_sum = zeros(1577,1577);
n_nodes = zeros(ThresholdTics,numreps);
n_edges = zeros(ThresholdTics,numreps);
% 
% modular = zeros(1,numreps);
%cc = zeros(1,numreps);
%avgdeg = zeros(ThresholdTics,numreps);
%otheravgdeg = zeros(ThresholdTics,numreps); 
apl = zeros(ThresholdTics,numreps);
% diams = zeros(1,numreps);

%DataExcludedTable = zeros(numreps, ThresholdTics);
...
for z = 1:numreps
    ...
    for i = 1:size(ThreshRange,2)
        network = ( abs( rho ) > ThreshRange(1,i) );
        n_nodes(i,z) = sum( (sum( network ) > 0));
        n_edges(i,z) = sum( sum( network ))./2;
        dist = graphallshortestpaths(sparse(network));
        dist1 = dist(~isinf(dist));
        dist2 = dist1(dist1 ~=0);
        apl(i,z) = mean(dist2);
        %avgdeg(i,z) = (2*n_edges(i,z))/n_nodes(i,z);
        %otheravgdeg(i,z) = (2*n_edges(i,z)/n_nodes(i,z));
        
        %degrees = sum(network)./2;
        %deg_sort = sort(degrees);
        %deg_max = ceil(max(degrees)) %NEED TO FIX THIS
...
%             hold on;
%         end
        %and just for fun, to show the power law:


        %DataExcludedTable(z,i) = (1577 - n_nodes)./1577;
    end;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %AVERAGE DEGREE
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%    avgdeg(z) = (2*n_edges(z))/1577;
% 
%     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     %DIAMETER
%     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     edge_list = [mod(nw_indices,1577), ceil(nw_indices./1577) ];
...
% pk = m.^(-1.63);
% pk = pk*2
% plot(m,pk,'YMinorTick','on')
% 
% 
% avgoverreps_right = mean(avgdeg');
% avgoverreps_wrong = mean(otheravgdeg');
% legend('Threshold = 0.1','Threshold = 0.19','Threshold = 0.28','Threshold = 0.37','Threshold = 0.46', 'Threshold = 0.55','Threshold = 0.64','Threshold = 0.73', 'p=k^-^a')
% set(gca,'FontSize',16)
% ylim([0.1 3])
%YTick([0.2 0.5 1.0 1.5 2.0])


% semilogy(ThreshRange,avgoverreps_right,'r-')
% hold on;
% semilogy(ThreshRange, avgoverreps_wrong,'b-')
% xlim([0 1])
% ylim([1 2500])
% legend('Including singletons','Discarding singletons')
% xlabel('Threshold')
% ylabel('Average degree')

%Save all of this output for future use.
% csvwrite('Method1_036thresh_2000reps_nnodes.csv',n_nodes)
% csvwrite('Method1_036thresh_2000reps_nedges.csv',n_edges)
% csvwrite('Method1_036thresh_2000reps_mod.csv',modular)
% csvwrite('Method1_036thresh_2000reps_cc.csv',cc)
% csvwrite('Method1_036thresh_2000reps_avgdeg.csv',avgdeg)
% csvwrite('Method1_036thresh_2000reps_apl.csv',apl)
% csvwrite('Method1_036thresh_2000reps_diams.csv',diams)
...
% y = hist(n_nodes, diff);
% why = y./sum(y);
% bar(min(n_nodes):max(n_nodes)-1,why)
% set(gca,'FontSize',14)
% xlabel('Number of nodes')
% ylabel('Frequency')
% title('Distribution of number of nodes, 2000 reps, rho = 0.36, true data')
...
% x = hist(n_nodes,diff);
% exe = x./sum(x);
% bar(min(n_edges):max(n_edges)-1,exe)
% set(gca,'FontSize',14)
% xlabel('Number of edges')
% ylabel('Frequency')
% title('Distribution of number of edges, 2000 reps, rho = 0.36, true data')
% 
...
% hold on;
% xlim([0 20])
% title('Distribution of diameters, 2000 reps, rho=0.36, true data')
%line([18 18], [0.1 500],'Color',[1 0 0])


%plot avg degrees
% avgdegmean = mean(avgdeg,2);
% semilogy(linspace(0,1,ThresholdTics),avgdegmean,'o-','LineWidth',2,'MarkerFace',[1 1 1])
% hold on;
% set(gca,'FontSize',24)
% xlabel('Threshold')
% ylabel('Average Degree')
...
%     linex(i) = 0.36;
% end;
% liney=linspace(1,1600,10);
% plot(linex,liney,'k--','LineWidth',4)

%plot avg path length
avgpathlen = mean(apl,2);
plot(linspace(0,1,ThresholdTics),avgpathlen,'o-','LineWidth',2,'MarkerFace',[1 1 1])
hold on;
set(gca,'FontSize',24)
xlabel('Threshold')
...
% ylabel('Frequency')
% title('Distribution of avg. path lengths, 2000 reps, rho=0.36, true data')
...
% ylabel('Frequency')
% title('Distribution of clustering coeff., 2000 reps, rho=0.36, true data')
% xlim([0 1])
...
% ylabel('Frequency')
% xlim([0 1])
% title('Distribution of modularity scores, 2000 reps, rho=0.36, true data')
